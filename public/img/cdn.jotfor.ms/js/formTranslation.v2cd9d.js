...
var FormTranslation = {
  ...
  getFormElements: function() {
    ...
    var spans = $$('span[id="coupon-header"], span[id="subtotal-text"], span[id="tax-text"], span[id="shipping-text"], span[id="total-text"], span.form-item-subtotal, span[id^="collapse-text"], span[class="span_print"], span[class="form-product-name"], span[class="form-product-child-label"], span[class^="multipleFileUploadLabels"], span.jfProduct-optionLabel');
    
    var inputs = $$('input');
    var textarea = $$('textarea');
    var buttons = $$('button:not(.form-submit-print):not(.form-submit-preview)');
    var h1 = $$('h1');
    var h2 = $$('h2');
    var h3 = $$('h3');
    ...
    var cardFormFileUploadElements = [
      ...
      '.jfUpload-subheading .minFileSize', '.jfUpload-subheading .maxFileSize',
    ];
    ...
    self.formElements = self.formElements.concat(labels, sublabels, inputs, h1, h2, h3,
                        welcomeHeader,subHeader, textarea, buttons, selects, ths, freeText,
                        spans, hover, multipleUploadButton, descriptions, newSublabels, jfDropdownOptions,
                        cardProgressItems, sliderItems, progressItems, cardFormItems, paymentItems, requiredLegends);
  },
  translate: function (word, compoundHint) {
    if (typeof word !== "string") { return; }
    var self = this;
    var tmpWord = word.replace(/^\s+|\s+$/g, '').strip();
    tmpWord = tmpWord.replace(/&gt;/g, '>').strip();
    tmpWord = tmpWord.replace(/&lt;/g, '<').strip();
    tmpWord = tmpWord.replace(/<br>/g, '').strip();
    tmpWord = tmpWord.replace(/\s+/gim, ' ').strip();
  ...
  translateFormElements: function(formElementList){
    ...
    $A(formElementList).each(function(e){
      if (e && e.hasClassName && e.hasClassName('noTranslate')) {
        ...
        if(!element.hasClassName('jfCheckbox-otherInputText') && !element.hasClassName('jfRadio-otherInputText')) {
        ...
        if (e.children.length > 0) {
          ...
          } else if (e.hasClassName('jfQuestion-label') || (e.children[1] && e.children[1].hasClassName('jfRequiredStar'))) {
